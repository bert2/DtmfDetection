namespace DtmfDetection {
    ...
    public readonly struct DtmfChange : IEquatable<DtmfChange> {
        ...
        public static DtmfChange Start(PhoneKey key, int channel) => new DtmfChange(key, new TimeSpan(), channel, isStart: true);

        public static DtmfChange Stop(PhoneKey key, int channel) => new DtmfChange(key, new TimeSpan(), channel, isStart: false);

        public override string ToString() => $"{(IsStart ? "Start: " : "Stop: ")} {Key} @ {Position} (ch: {Channel})";

        #region Equality implementations

        public bool Equals(DtmfChange other) => Key == other.Key && Position.Equals(other.Position) && Channel == other.Channel && IsStart == other.IsStart;
