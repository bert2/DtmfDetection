namespace DtmfDetection {
    ...
    /// <summary>Convenience implementation of `ISamples` for PCM audio data.
    ...
    public class AudioData : ISamples {
        ...
        /// <summary>Creates a new `AudioData` from the given array of `float` values which were sampled with the given sample rate and for
        /// the given number of channels.</summary>
        /// <param name="samples">An array of `float`s representing the PCM data.</param>
        /// <param name="channels">The number of channels in the PCM data. If this value is greater than `1` then the sample values in `samples`
        /// must be interleaved (i.e. `left sample 1, right sample 1, left sample 2, right sample 2, ...`).</param>
        /// <param name="sampleRate">The sample rate of the PCM data in Hz. This should match the sample rate the `Analyzer` is
        ...
        /// <summary>Reads `count` samples from the input and writes them into `buffer`. Because the input PCM data already has the expected
        /// format, this boils down to a simple call to `Array.Copy()`.</summary>
