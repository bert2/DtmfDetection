namespace DtmfDetection {
    ...
    public readonly struct Config : IEquatable<Config> {
        ...
        /// <summary>The default number of samples to analyze before the Goertzel response should be calulated (tuned to minimize
        /// error of the target frequency bin).</summary>
        ...
        /// <summary>The detection threshold. Typical values are `30`-`35` (when `NormalizeResponse` is `true`) and `100`-`115`
        /// (when `NormalizeResponse` is `false`).</summary>
        public readonly double Threshold;

        /// <summary>The number of samples to analyze before the Goertzel response should be calulated. It is recommened to leave it
        /// at the default value `205` (tuned to minimize error of the target frequency bin).</summary>
        public readonly int SampleBlockSize;

        /// <summary>The sample rate (in Hz) the Goertzel algorithm expects. Sources with higher samples rates must resampled to this
        /// sample rate. It is recommended to leave it at the default value `8000`.</summary>
        public readonly int SampleRate;

        /// <summary>Toggles normalization of the Goertzel response with the total signal energy of the sample block. Recommended
        /// setting is `true` as this provides invariance to loudness changes of the signal.</summary>
        public readonly bool NormalizeResponse;

        /// <summary>Creates a new `Config` instance.</summary>
        /// <param name="threshold">The detection threshold. Typical values are `30`-`35` (when `normalizeResponse` is `true`)
        /// and `100`-`115` (when `normalizeResponse` is `false`).</param>
        /// <param name="sampleBlockSize">The number of samples to analyze before the Goertzel response should be calulated. It is
        /// recommened to leave it at the default value `205` (tuned to minimize error of the target frequency bin).</param>
        /// <param name="sampleRate">The sample rate (in Hz) the Goertzel algorithm expects. Sources with higher samples rates must
        /// resampled to this sample rate. It is recommended to leave it at the default value `8000`.</param>
        /// <param name="normalizeResponse">Toggles normalization of the Goertzel response with the total signal energy of the sample
        /// block. Recommended setting is `true` as this provides invariance to loudness changes of the signal.</param>
