namespace DtmfDetection.NAudio {
    ...
    /// <summary>Convenience implementation of `ISamples` for an infinite `IWaveIn` audio stream.
    ...
    public class AudioStream : ISamples {
        ...
        /// <summary>Creates a new `AudioStream` from an `IWaveIn` input by buffering it with a `BufferedWaveProvider`.
        ...
        /// <param name="targetSampleRate">Used to resample the `IWaveIn` input. This should match the sample rate (in Hz) the `Analyzer` is
        ...
        /// <summary>Reads `count` samples from the input and writes them into `buffer`.
        /// Will block as long as it takes for the input to buffer the requested number of samples.</summary>
        /// <param name="buffer">The output array to write the read samples to.</param>
        /// <param name="count">The number of samples to read.</param>
        /// <returns>The number of samples that have been read. Will always equal `count` except when `StopWaiting()` has been called,
        /// in which case `Read()` returns `0`.</returns>
        ...
        /// <summary>Stops waiting for the input to buffer data. Some `IWaveIn`s don't have data available continuously.
        /// For instance a `WasapiLoopbackCapture` will only have data as long as the OS is playing some audio.
        /// Calling `StopWaiting()` will break the infinite wait loop and the `Analyzer` processing this `AudioStream` will consider
        /// it being "finished". This in turn helps to gracefully exit the thread running the analysis.</summary>
