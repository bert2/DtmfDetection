...
Convenience implementation of `ISamples` for `WaveStream` audio. `WaveStream`s are commonly used for finite data like audio files.
...
| Name | Summary |
|---|---|
| **AudioFile(WaveStream source, int targetSampleRate, bool forceMono)** | Creates a new `AudioFile` from a `WaveStream` input. |
## Methods
...
Convenience implementation of `ISamples` for an infinite `IWaveIn` audio stream.
...
| Name | Summary |
|---|---|
| **AudioStream(IWaveIn source, int targetSampleRate, bool forceMono)** | Creates a new `AudioStream` from an `IWaveIn` input by buffering it with a `BufferedWaveProvider`. |
## Methods
...
Helper that does audio analysis in a background thread.

## Constructors

| Name | Summary |
|---|---|
| **BackgroundAnalyzer(IWaveIn source, bool forceMono, Config? config, IAnalyzer analyzer)** | Creates a new `BackgroundAnalyzer` and immediately starts listening to the `IWaveIn` input. |
## Methods
...
| Name | Returns | Summary |
|---|---|---|
| **WaitForSamples(BufferedWaveProvider source, int count)** | bool | Blocks the thread for as long as the `BufferedWaveProvider` minimally should need to buffer at least `count` sample frames. |
# IWaveInExt Class
...
| Name | Type | Summary |
|---|---|---|
| **WaveFormat** | WaveFormat | The `WaveFormat` of the decorated `ISampleProvider`. |
## Constructors
...
| **Resample(ISampleProvider source, int targetSampleRate)** | ISampleProvider | Resamples the input data to the specified target sample rate using the `WdlResamplingSampleProvider`. |
# WaveStreamExt Class
